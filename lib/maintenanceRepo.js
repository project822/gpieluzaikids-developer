...
// Cache singkat (2 detik) — halaman dashboard & system mem-poll endpoint ini
// (5–10 detik) dan bisa terbuka bersamaan di banyak tab; cache menghindari
// pembacaan MongoDB berulang yang tidak perlu. Selalu di-invalidate saat tulis.
let cache = { at: 0, status: null };
const MAINT_CACHE_MS = 2000;

function invalidateCache() {
  cache = { at: 0, status: null };
}
...
export async function getMaintenanceStatus() {
  const now = Date.now();
  if (cache.status && now - cache.at < MAINT_CACHE_MS) return cache.status;
